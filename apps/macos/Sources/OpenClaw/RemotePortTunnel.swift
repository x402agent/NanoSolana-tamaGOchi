...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.nanosolana", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = NanoSolanaConfigFile.hostKey(sshHost)
        let urlKey = NanoSolanaConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.nanosolana.remote.tunnel.port", qos: .utility)
            do {
