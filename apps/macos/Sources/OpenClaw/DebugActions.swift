...
enum DebugActions {
    private static let verboseDefaultsKey = "nanosolana.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "nanosolana.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = NanoSolanaPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "NanoSolana", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = NanoSolanaPaths.configURL
        guard
