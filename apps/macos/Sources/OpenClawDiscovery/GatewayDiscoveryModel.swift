...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.nanosolana", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in NanoSolanaBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: NanoSolanaBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.nanosolana.macos.gateway-discovery",
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = NanoSolanaBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = NanoSolanaBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (NanoSolana)", with: "")
