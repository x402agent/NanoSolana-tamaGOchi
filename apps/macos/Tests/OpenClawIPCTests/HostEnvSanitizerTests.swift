...
struct HostEnvSanitizerTests {
    ...
    @Test func `sanitize shell wrapper allows only explicit override keys`() {
        ...
        #expect(env["LANG"] == "C")
        #expect(env["LC_ALL"] == "C")
        #expect(env["NANOSOLANA_TOKEN"] == nil)
        #expect(env["PS4"] == nil)
    }

    @Test func `sanitize non shell wrapper keeps regular overrides`() {
        let env = HostEnvSanitizer.sanitize(overrides: ["NANOSOLANA_TOKEN": "secret"])
        #expect(env["NANOSOLANA_TOKEN"] == "secret")
    }
}
