...
import { sleep } from "nanosolana/plugin-sdk/voice-call";
...
function resolveDefaultStorePath(config: VoiceCallConfig): string {
  const preferred = path.join(os.homedir(), ".nanosolana", "voice-calls");
...
export function registerVoiceCallCli(params: {
  ...
  const root = program
    ...
    .addHelpText("after", () => `\nDocs: https://docs.nanosolana.ai/cli/voicecall\n`);
