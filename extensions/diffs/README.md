...
Read-only diff viewer plugin for **NanoSolana** agents.
...
Set plugin-wide defaults in `~/.nanosolana/nanosolana.json`:
...
Before:
NanoSolana supports plugins.

After:
NanoSolana supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/nanosolana-diffs`).
