/**
 * Twitch channel plugin for NanoSolana.
 *
...
import { buildChannelConfigSchema } from "nanosolana/plugin-sdk/twitch";
 ...
 * for NanoSolana. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: NanoSolanaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: NanoSolanaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: NanoSolanaConfig): boolean => {
