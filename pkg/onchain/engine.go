// Package onchain provides NanoSolana's on-chain financial engine.
//
// Powered by the gagliardetto/solana-go SDK + Helius RPC/WSS for real-time
// mainnet data. This is the agent's direct connection to the Solana blockchain.
//
// Capabilities:
//   - Real-time balance & token portfolio queries (Helius RPC)
//   - Transaction history with enhanced parsing (Helius API)
//   - WebSocket account monitoring for live position tracking (Helius WSS)
...
//
// All RPC calls go through the user's Helius endpoint for reliability +
// enhanced data (DAS, parsed transactions, priority fees).
package onchain
...
// Config holds all Helius + Solana connection settings.
type Config struct {
...
func (c Config) Validate() error {
	if c.HeliusRPCURL == "" {
		return fmt.Errorf("HELIUS_RPC_URL is required — get one at https://helius.dev")
	}
	if c.HeliusAPIKey == "" {
		return fmt.Errorf("HELIUS_API_KEY is required — get one at https://helius.dev")
	}
	return nil
}
...
// Engine is NanoSolana's on-chain financial engine.
type Engine struct {
	cfg    Config
	rpc    *rpc.Client
	wsConn *ws.Client
...
// NewEngine creates a new on-chain engine connected to Helius.
func NewEngine(cfg Config) (*Engine, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}

	rpcClient := rpc.New(cfg.HeliusRPCURL)
...
func (e *Engine) ConnectWSS(ctx context.Context) error {
	if e.cfg.HeliusWSSURL == "" {
		e.logf("⚠️  No HELIUS_WSS_URL — real-time subscriptions disabled")
		return nil
	}

	wsClient, err := ws.Connect(ctx, e.cfg.HeliusWSSURL)
	if err != nil {
		return fmt.Errorf("WSS connect: %w", err)
	}
	e.wsConn = wsClient
	e.logf("🔌 WSS connected to Helius")
	return nil
}
...
func (e *Engine) SendSOL(
	...
	if e.cfg.HeliusWSSURL != "" {
		wsClient, wErr := ws.Connect(ctx, e.cfg.HeliusWSSURL)
		if wErr == nil {
...
func (e *Engine) GetPriorityFees(ctx context.Context) (*PriorityFeeEstimate, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", e.cfg.HeliusRPCURL, strings.NewReader(payload))
	if err != nil {
		return nil, err
	}
...
func (e *Engine) WatchAccount(ctx context.Context, pubkey solanago.PublicKey) (<-chan uint64, error) {
	if e.wsConn == nil {
		return nil, fmt.Errorf("WSS not connected — set HELIUS_WSS_URL")
	}
