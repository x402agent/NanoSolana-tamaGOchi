...
//
// NanoSolana Seeker agent — runs the full trading loop on the Solana
...
// Agent is the NanoSolana Seeker agent running on the phone.
type Agent struct {
	cfg    SeekerConfig
	bridge *BridgeClient
...
func (a *Agent) Run(ctx context.Context) error {
	...
	a.logf("🦞 NanoSolana Seeker starting...")
	...
	a.logf("")
	a.logf("  ┌──────────────────────────────────────────┐")
	a.logf("  │  🐹 NanoSolana Seeker — Agent Active      │")
	a.logf("  │  OODA Loop · Jupiter · Helius · Telegram   │")
	a.logf("  │  10MB Go Binary · ARM64 · Solana Seeker    │")
	a.logf("  └──────────────────────────────────────────┘")
	a.logf("")
	a.logf("  Start the full daemon with: nanosolana daemon")
	a.logf("  Or OODA only:               nanosolana ooda --sim")
	a.logf("")

	// ── 7. Block until shutdown ──────────────────────────────
	<-ctx.Done()
	a.logf("🛑 NanoSolana Seeker shutting down...")
...
## Identity
- You are a NanoSolana trading agent running on a Solana Seeker phone
...
## Commands
- nanosolana solana health — check RPC status
- nanosolana solana balance — check wallet
- nanosolana ooda --sim — paper trading
- nanosolana ooda --interval 60 — live trading (60s cycles)
